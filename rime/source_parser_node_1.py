    ...
    if (ser.isOpen()):
            ...
            if(message_indicator == "aa" or message_indicator == "55"):
                ...
                if  count == 5: 
                    ...
                    # print opt_iter
                    # # print A['Node1'].message.decision_T[0:2]
                    # print output_dic['T_n'][0][:,opt_iter+1] 
                    # print output_dic['T_n'][1][:,opt_iter+1]
                    # print output_dic['T_s'][0][:,opt_iter+1] 

                    input_dic['T_n'][0][:,opt_iter+1] = ( output_dic['T_n'][0][:,opt_iter+1]*1000 ).astype(int)  
                    input_dic['etaa'][0][:,opt_iter+1] = ( output_dic['etaa'][0][:,opt_iter+1]*1000 ).astype(int)  
                    input_dic['uss'][0][:,opt_iter+1] = ( output_dic['uss'][0][:,opt_iter+1]*1000 ).astype(int)  
                    input_dic['slacks_2'][0][:,opt_iter+1] = ( output_dic['slacks_2'][0][:,opt_iter+1]*1000 ).astype(int)  
                    input_dic['slacks'][0][:,opt_iter+1] = ( output_dic['slacks'][0][:,opt_iter+1]*1000 ).astype(int)  
                    # input_dic['Y_soln'][0][:,opt_iter] =   
                    input_dic['mu_cons2'][0:10,opt_iter+1] = ( output_dic['mu_cons2'][0:10,opt_iter+1]*100 ).astype(int)  
                    input_dic['mu_cons1'][0:1:,opt_iter+1] = ( output_dic['mu_cons1'][0:1:,opt_iter+1]*100 ).astype(int)  
                    input_dic['mu'][0:1,opt_iter+1] = ( output_dic['mu'][0:1,opt_iter+1]*100 ).astype(int)  

                    A['Node1'] = dummy
                    A['Node1'].message.decision_T[0:2] = input_dic['T_n'][node_id][:,opt_iter+1].astype(int).tolist()   
                    A['Node1'].message.eta = np.asscalar( input_dic['etaa'][node_id][:,opt_iter+1].astype(int) ) 
                    A['Node1'].message.uss = input_dic['uss'][node_id][:,opt_iter+1].astype(int).tolist()  
                    A['Node1'].message.slacks2 = input_dic['slacks_2'][node_id][:,opt_iter+1].astype(int).tolist()     
                    A['Node1'].message.slacks = input_dic['slacks'][node_id][:,opt_iter+1].astype(int).tolist()       
                    # A['Node1'].message.Y_soln[0:2] = input_dic['Y_soln'][node_id][:,opt_iter+1].astype(int).tolist()   
                    A['Node1'].message.mu_cons2 = input_dic['mu_cons2'][0:10,opt_iter+1].astype(int).tolist()     
                    A['Node1'].message.mu_cons1 = np.asscalar( input_dic['mu_cons1'][0:1,opt_iter+1].astype(int) )  
                    A['Node1'].message.mu = np.asscalar( input_dic['mu'][0:1,opt_iter+1].astype(int) )  
                    ...
                    if (ser.isOpen()):
                        ...
                        print "Wrote to serial port ( primal update: source 1 )"
                    opt_iter = opt_iter + 1
                ...
                if count == 10:
                    ...
                    output_dic = source1_dual.algorithm(node_id, opt_iter, input_dic)
                    ...
                    input_dic['T_n'][0][:,opt_iter] = ( output_dic['T_n'][0][:,opt_iter]*1000 ).astype(int)  
                    input_dic['etaa'][0][:,opt_iter] = ( output_dic['etaa'][0][:,opt_iter]*1000 ).astype(int)  
                    input_dic['uss'][0][:,opt_iter] = ( output_dic['uss'][0][:,opt_iter]*1000 ).astype(int)  
                    input_dic['slacks_2'][0][:,opt_iter] = ( output_dic['slacks_2'][0][:,opt_iter]*1000 ).astype(int)  
                    input_dic['slacks'][0][:,opt_iter] = ( output_dic['slacks'][0][:,opt_iter]*1000 ).astype(int)  
                    # input_dic['Y_soln'][0][:,opt_iter] =   
                    input_dic['mu_cons2'][0:10,opt_iter] = ( output_dic['mu_cons2'][0:10,opt_iter]*100 ).astype(int)  
                    input_dic['mu_cons1'][0:1:,opt_iter] = ( output_dic['mu_cons1'][0:1:,opt_iter]*100 ).astype(int)  
                    input_dic['mu'][0:1,opt_iter] = ( output_dic['mu'][0:1,opt_iter]*100 ).astype(int)  

                    A['Node1'] = dummy
                    A['Node1'].message.decision_T[0:2] = input_dic['T_n'][node_id][:,opt_iter].astype(int).tolist()   
                    A['Node1'].message.eta = np.asscalar( input_dic['etaa'][node_id][:,opt_iter].astype(int) ) 
                    A['Node1'].message.uss = input_dic['uss'][node_id][:,opt_iter].astype(int).tolist()  
                    A['Node1'].message.slacks2 = input_dic['slacks_2'][node_id][:,opt_iter].astype(int).tolist()     
                    A['Node1'].message.slacks = input_dic['slacks'][node_id][:,opt_iter].astype(int).tolist()       
                    # A['Node1'].message.Y_soln[0:2] = input_dic['Y_soln'][node_id][:,opt_iter+1].astype(int).tolist()   
                    A['Node1'].message.mu_cons2 = input_dic['mu_cons2'][0:10,opt_iter].astype(int).tolist()     
                    A['Node1'].message.mu_cons1 = np.asscalar( input_dic['mu_cons1'][0:1,opt_iter].astype(int) )  
                    A['Node1'].message.mu = np.asscalar( input_dic['mu'][0:1,opt_iter].astype(int) ) 
                    ...
                    if (ser.isOpen()):
                        ser.write(motemessage2.build(parsed_data).encode('hex')+"\n")
                        print "Wrote to serial port ( dual update: source 1 )"
                    ...
                    max_constraints_2[0,opt_iter-1] = output_dic['max_constraints_2'][0,opt_iter-1]     
                    max_constraints_1[0,opt_iter-1] = output_dic['max_constraints_1'][0,opt_iter-1]  
                    max_sum_constraints[0,opt_iter-1] = output_dic['max_sum_constraints'][0,opt_iter-1]  
                    obj_cvar_adal[0,opt_iter-1]  = output_dic['obj_cvar_adal'][0,opt_iter-1]  
   
                    print obj_cvar_adal[0,opt_iter-1] 
                    # f = open('obj_cvar_adal.txt', 'w') 
                    # f.write(str( obj_cvar_adal ) )       
                    
                    print max_constraints_2[0,opt_iter-1] 
                    # f = open('max_constraints_2.txt', 'w') 
                    # f.write(str( max_constraints_2 ) )     

                    print max_constraints_1[0,opt_iter-1] 
                    # f = open('max_constraints_1.txt', 'w') 
                    # f.write(str( max_constraints_1 ) )  

                    print max_sum_constraints[0,opt_iter-1]
                    # f = open('max_sum_constraints.txt', 'w') 
                    # f.write(str( max_sum_constraints ) )  

                    if opt_iter == 300:
                        plt.figure(1)    
                        plt.plot(np.arange(opt_iter).reshape(1,opt_iter),obj_cvar_adal,'.b' )
                        plt.figure(2)
                        plt.plot(np.arange(opt_iter).reshape(1,opt_iter),max_constraints_2,'bo')
                        plt.plot(np.arange(opt_iter).reshape(1,opt_iter),max_constraints_1,'rs')
                        plt.plot(np.arange(opt_iter).reshape(1,opt_iter),max_sum_constraints,'g^')
                        plt.yscale('log')
                        plt.show() 

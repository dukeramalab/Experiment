...
def algorithm(T1, T2):
    ...
    for opt_iter in range(max_iter_adal):
        ...
        for i in range(J):
            ...
            T_n[i][:,opt_iter+1] = ( ( T_n[i][:,opt_iter] + tau*( Z[0:J,0] - T_n[i][:,opt_iter] ) )*1000 ).astype(int)  
            etaa[i][:,opt_iter+1] = ( ( etaa[i][:,opt_iter] + tau*( Z[J+J+1,0] - etaa[i][:,opt_iter] ) )*1000 ).astype(int)
            uss[i][:,opt_iter+1] = ( ( uss[i][:,opt_iter] + tau*( Z[J+J+1+1:J+J+1+1+Ns,0] - uss[i][:,opt_iter] ) )*1000 ).astype(int)
            slacks_2[i][:,opt_iter+1] = ( ( slacks_2[i][:,opt_iter] + tau*( Z[J+J+1+1+Ns:J+J+1+1+Ns+Ns,0] - slacks_2[i][:,opt_iter] ) )*1000 ).astype(int)
            slacks[i][:,opt_iter+1] = ( ( slacks[i][:,opt_iter] + tau*( Z[J+J+1+1+Ns+Ns:,0] - slacks[i][:,opt_iter] ) )*1000 ).astype(int)   

            T_n[i][:,opt_iter+1] = ( T_n[i][:,opt_iter+1] ).astype(float)/1000     
            etaa[i][:,opt_iter+1] = ( etaa[i][:,opt_iter+1] ).astype(float)/1000
            uss[i][:,opt_iter+1] = ( uss[i][:,opt_iter+1] ).astype(float)/1000 
            slacks_2[i][:,opt_iter+1] = ( slacks_2[i][:,opt_iter+1] ).astype(float)/1000  
            slacks[i][:,opt_iter+1] = ( slacks[i][:,opt_iter+1] ).astype(float)/1000   
        ...
        for i in range(K):
            ...
            T_s[i][:,opt_iter+1] = ( ( T_s[i][:,opt_iter] + tau*( Z[0:J,0] - T_s[i][:,opt_iter] ) )*1000 ).astype(int)
            Y_soln[i][:,opt_iter+1] = ( ( Z[J+J+1:,0] )*1000 ).astype(int)

            T_s[i][:,opt_iter+1] = ( T_s[i][:,opt_iter+1] ).astype(float)/1000
            Y_soln[i][:,opt_iter+1] = ( Y_soln[i][:,opt_iter+1] ).astype(float)/1000
        ...
        mu[:,opt_iter+1] = ( ( mu[:,opt_iter] + tau*rho*sum_constraints[:,opt_iter] )*100 ).astype(int)
        mu[:,opt_iter+1] = mu[:,opt_iter+1].astype(float)/100
        # End sum flow
        ...
        mu_cons2[:,opt_iter+1] = ( ( mu_cons2[:,opt_iter] + tau*rho*constraints_2[:,opt_iter] )*100 ).astype(int)  
        mu_cons2[:,opt_iter+1] = ( mu_cons2[:,opt_iter+1] ).astype(float)/100   
        # End network risk
        ...
        mu_cons1[:,opt_iter+1] = ( ( mu_cons1[:,opt_iter] + tau*rho*constraints_1[:,opt_iter] )*100 ).astype(int)
        mu_cons1[:,opt_iter+1] = ( mu_cons1[:,opt_iter+1] ).astype(float)/100
        # End network mean
        ...
        Output['etaa'] = etaa  
        Output['uss'] = uss   
        Output['slacks_2'] = slacks_2    
        Output['slacks'] = slacks    
        Output['Y_soln'] = Y_soln   
        Output['mu_cons2'] = mu_cons2    
        Output['mu_cons1'] = mu_cons1    
        Output['mu'] = mu   
